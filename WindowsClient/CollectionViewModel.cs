...
namespace Overmind.ImageManager.WindowsClient
{
	public class CollectionViewModel : INotifyPropertyChanged, IDisposable
	{
		public CollectionViewModel(CollectionModel model)
		{
			...
			ImageCollection = new ObservableCollection<ImageViewModel>(model.Images.Select(image => new ImageViewModel(image)));
		...
		private void AddImage(Uri uri)
		{
			...
			else
			{
				...
				ImageViewModel newImageViewModel = new ImageViewModel(newImage);
